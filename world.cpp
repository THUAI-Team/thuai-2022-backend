...
typedef struct b2bodydata {
    ...
    b2bodydata(Egg* _egg, b2Body* _b2body)
    ...
    // ========== Box2d Related ==========
    {
        ...
        for (int i = 0; i < GROUND_POLYGON_VERTEX_COUNT; i++) {
            ...
            if ((i % (GROUND_POLYGON_VERTEX_COUNT / 3)) <
                ...
                ve[i].Set(static_cast<float>((DIAMETER / 2) *
                                             cos(i / vertex_cnt_f * 2 * pi)),
...
void World::read_from_team_action(Team team, nlohmann::json detail) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!player_action["drop"].is_null()) {
            ...
            Vec2D pos_to_be_placed = {
                b2players[player_id]->GetPosition().x +
            ...
            for (auto egg : b2eggs) {
                if (egg == nullptr)
                    continue;
                if (get_distance(egg, pos_to_be_placed) <=
    ...
    try {
        ...
        for (int i = 0; i < PLAYER_COUNT; i++) {
            ...
            if (currentPlayer->status() == PlayerStatus::WALKING) {
                ...
                if (egg_holding != -1) {
                    walk_speed =
                        get_walk_speed_with_egg(eggs[egg_holding]->score());
                }
                b2currentPlayer->SetLinearVelocity(
            ...
            if (isSpeedDown) {
                b2currentPlayer->SetLinearVelocity(
                    {static_cast<float>(std::min(
                     ...
                     static_cast<float>(std::min(
                         currentPlayer->facing().y * SPEED_ON_SPEED_REDUCE,
                         static_cast<double>(
        ...
        for (int i = 0; i < EGG_COUNT; i++) {
            ...
            eggs[i]->set_velocity({b2eggs[i]->GetLinearVelocity().x,
                                   b2eggs[i]->GetLinearVelocity().y});
            if (get_distance(b2eggs[i], {0, 0}) > DIAMETER / 2) {
                double eggangle =
                    atan2(static_cast<float>(b2eggs[i]->GetPosition().y),
